...
BinarySearchTree.prototype.findInOrderSuccessor = function(inputNode) {
  var rootNode = findRoot(inputNode);
  var result = traverse(rootNode, inputNode, [rootNode]);
...
function traverse(node, target, closerNodes){
  if(node === null){
    return;
  }
  
  var diffValue = node.key - target.key;
  var closerValue = closerNodes[closerNodes.length - 1].key;
  if(diffValue > 0 && diffValue < (closerValue - target.key) && (node.key != target.key)){
    closerNodes.push(node);
  }

  traverse(node.left, target, closerNodes);
  traverse(node.right, target, closerNodes);
  
  return closerNodes[closerNodes.length - 1];
}
